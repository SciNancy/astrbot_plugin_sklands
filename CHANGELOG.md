...
- **`/sk gacha`**：
  - 从 nonebot-plugin-sklands 移植完整的明日方舟抽卡记录获取、分组、渲染流程
  - 支持从官网 API 拉取全部抽卡历史，自动分页并发获取
  - 按卡池分组统计，自动计算 UP 角色、歪卡率、六星出货率
  - 支持渲染为多张图片（每页最多 30 个卡池）
  - 新记录自动去重后存入数据库，避免重复拉取
- **`/sk efgacha`**：
  - 移植终末地抽卡记录获取与渲染
  - 支持 STANDARD / SPECIAL / BEGINNER / WEAPON 四类卡池
  - 按类别分组，支持分多张图片渲染（每页最多 5 个卡池）
- **数据库**：
  - 新增 `skland_gacha_record` 表，存储抽卡记录
  - 支持 `arknights` / `endfield` 两种游戏类型
- **卡池数据**：
  - 插件启动时自动加载卡池数据（`gacha_table_data.load()`）
  - 用于解析卡池 UP 角色和开放时间
